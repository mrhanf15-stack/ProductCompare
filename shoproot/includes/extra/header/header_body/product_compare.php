<?php
/* -----------------------------------------------------------------------------------------
   Product Compare v2.0.0 - Header Body Hookpoint
   ...
   v2.0.0: BUGFIX - Cookie-Restore nur wenn Cookie tatsächlich existiert
           - Nach Clear wird das Cookie gelöscht (product_compare.php + AJAX)
           - Dadurch greift der Restore hier nicht mehr nach einem Clear
           - Zusätzliche Sicherheit: Prüfe ob Cookie noch im aktuellen Request existiert
   ...
   v1.0.1: DIR_FS_LANGUAGES → DIR_WS_LANGUAGES
...
if (defined('MODULE_PRODUCT_COMPARE_STATUS') && MODULE_PRODUCT_COMPARE_STATUS == 'true') {
    ...
    // HINWEIS: Nach einem Clear wird das Cookie gelöscht, daher greift dieser
    // Block dann NICHT mehr (isset($_COOKIE['pc_compare_ids']) = false)
